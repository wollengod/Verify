...
class OfficePropertyPage extends StatefulWidget {
  const OfficePropertyPage({super.key});

  @override
  State<OfficePropertyPage> createState() => _OfficePropertyPageState();
}

class _OfficePropertyPageState extends State<OfficePropertyPage> {
  ...
  Future<List<OfficePropertyModel>> fetchOfficeProperties() async {
    ...
    final url = Uri.parse(
      "https://verifyrealestateandservices.in/Second%20PHP%20FILE/main_application/office.php?user_id=$userId",
    );
    ...
    if (response.statusCode == 200) {
      ...
      final List<dynamic> data = jsonData['data']; // <--- access the list inside "data"
              ...
              if (data == null || data.isEmpty) {
                return const Center(
                    child: Column(
                      children: [
                        SizedBox(height: 100,),
                        Text("No Office properties found.",style: TextStyle(color: Colors.black)),
                      ],
                    )
                );
              }
